...
package Plugins::VirtualLibraryCreator::ConfigManager::ParameterHandler;
...
sub addValuesToTemplateParameter {
	...
	if ($p->{'type'} =~ '^sql.*') {
		...
		my $listValues = $cache->get('vlc_contributorlist_all') || [];
		$p->{'values'} = $listValues;
	} elsif ($p->{'type'} eq 'contributorlistcachedalbumartists') {
		my $listValues = $cache->get('vlc_contributorlist_albumartists') || [];
		$p->{'values'} = $listValues;
	} elsif ($p->{'type'} eq 'contributorlistcachedcomposers') {
		my $listValues = $cache->get('vlc_contributorlist_composers') || [];
		$p->{'values'} = $listValues;
	} elsif ($p->{'type'} eq 'genrelistcached') {
		my $listValues = $cache->get('vlc_genrelist') || [];
		$p->{'values'} = $listValues;
	} elsif ($p->{'type'} eq 'contenttypelistcached') {
		my $listValues = $cache->get('vlc_contenttypes') || [];
		$p->{'values'} = $listValues;
	} elsif ($p->{'type'} eq 'releasetypelistcached') {
		my $listValues = $cache->get('vlc_releasetypes') || [];
		$p->{'values'} = $listValues;
...
sub getSQLTemplateData {
	...
	for my $sql (split(/[;]/, $sqlstatements)) {
		...
		if ($@) {
			$log->warn("Running: $sqlstatements got error:\n".$@);
		}
	}
	return \@result;
}
